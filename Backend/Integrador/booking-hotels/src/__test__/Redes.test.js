...
describe('<Redes />', () => {
    ...
    it('Existen 4 links', () => {
        const view = render(<MemoryRouter> <Redes /> </MemoryRouter>);
    ...
    it('Cada ícono tiene su link correspondiente', () => {
        ...
        for(let i = 0; i <= 3; i++) {
            ...
            if (iconoElemento.classList.contains("fa-facebook"))
                expect(iconoElemento.closest('a')).toHaveAttribute('href', 'https://facebook.com');
            if (iconoElemento.classList.contains("fa-linkedin"))
                expect(iconoElemento.closest('a')).toHaveAttribute('href', 'https://linkedin.com');
            if (iconoElemento.classList.contains("fa-instagram"))
                expect(iconoElemento.closest('a')).toHaveAttribute('href', 'https://instagram.com');
            if (iconoElemento.classList.contains("fa-twitter"))
                expect(iconoElemento.closest('a')).toHaveAttribute('href', 'https://twitter.com');
        }
    });
});