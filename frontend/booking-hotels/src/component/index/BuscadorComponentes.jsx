...
import "../../styles/newProduct/buscadorCity.css";
...
const selectFormatOptionLabel = ({ value, label }) => (
  <div className={stylesbuscador.select_option_list}>
    <i
      className={`far fa-map-marker-alt fa-lg ${stylesbuscador.select_icono_mapa_vacio} select_icono_mapa_vacio`}
      aria-hidden="true"
    />
...
export const SelectMenu = ({
  ...
  useEffect(() => {
    ...
    .then((data) => setOptions(listadoEnFormatoSelect(data)))
    .catch((e) => {
  ...
  return (
    <div className={`${stylesbuscador.select_color}  ${customClass}`}>
      <div
        className={
          selectedCity === ""
      ...
      <Select
        ...
        className={
          selectedCity
            ? `${stylesbuscador.oculta}`
            : `${stylesbuscador.select_container} ${className}`
        }
        name={name}
        id={id}
        options={options}
